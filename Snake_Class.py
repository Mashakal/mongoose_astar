...
The snake is composed of three Benzier curves that are connected at the 
endpoints 

The snake's Movment is facilitaed by vector addtion that preformed in SnakeFSM
...
class Snake (pygame.sprite.Sprite):
	def __init__(self, A, B, C, D, E, F, G):
		...
		self.head_circle = pygame.Rect(self.A[0]-20, self.A[1]-20, 20*2, 20*2)
		
		self.FSM = SnakeFSM.SnakeFSM(self)
	def collision(self):
		if self.head_circle.colliderect((self.goal[0], self.goal[1], 35, 35)) == True:
			print "GOT-CHA"
			return True
		else:
			return False 

	def getDistance(self, baby_list):
		smallest = 35 + 19
		nearest = None
		i = 0
		while i <= len(baby_list) - 1:
			temp_vect = Vector_Class.Vector((self.A[0] / 35, self.A[1] / 35 ), (baby_list[i][0], baby_list[i][1]))
			distance = (temp_vect.x + temp_vect.y) 
			if distance < smallest:
				smallest = distance
				nearest = [baby_list[i][0], baby_list[i][1]]
			i +=1
		self.goal = nearest
		return nearest

	def getGoalGrid(self, goal):
		pixal_goal = [goal[0] * 35, (goal[1] * 35) + 50]
		self.goal = pixal_goal
		return pixal_goal
	...
	def draw_head(self):
		...
		#self.head_surface.fill(WHITE)
		#self.head_surface.set_colorkey(WHITE)
		pygame.draw.ellipse(self.head_surface, BROWN, (self.headx + 30, self.heady + 8, 40, 20))
	...
	def draw_snake(self):
		...
		degrees = self.getAngle(self.headx, self.heady, self.A[0], self.A[1])
		if self.FSM.cur_state_name == "Traveling":
			degrees = self.getAngle(self.headx, self.heady, self.A[0], self.A[1])
			self.rotate(self.headx, self.heady, degrees)
		elif self.FSM.cur_state_name == "Offense":
...
The snake's screen, goal and state are also initialized below.
"""
snake = Snake(A, B, C, D, E, F, G)
#snake.screen = screen
#snake.goal = goal
